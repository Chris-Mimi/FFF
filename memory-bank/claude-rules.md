...
`bookings`, `weekly_sessions`, `wods`, `wod_section_results`, `lift_records`, `benchmark_results`, `reactions`, `athlete_achievements`, `programming_plan_items`, **`exercises`** (added S408 — a catalogue, not a log, so it grows slowly, but already at **716/1000** and every read of it is unfiltered. **Swept in S409:** all 11 read sites now go through `fetchAllExercises()` in `utils/fetch-all-exercises.ts`, which pages past the cap. **Use that helper for any new read of this table** — don't hand-roll `.from('exercises').select()`). As of 2026-05-13 the gym has ~2,000 bookings and grows ~330/month, so every one of these tables either has crossed or will cross 1,000 rows within months.
...
### Coach-only data must NOT be a column on an owner-readable table (S407)
**Why:** S407 — park/block "reasons" were added as columns on `members`. But the members SELECT policy is `auth.uid() = id OR auth.uid() = primary_member_id`, so an athlete can read **their own** row — every column. A *parked* athlete keeps full app access, so they could fetch their own reason via the API/devtools even though the UI never showed it. Column-level hiding is impossible here: coaches and athletes share the Postgres `authenticated` role (the app distinguishes them only by the JWT `user_metadata.role = 'coach'` flag), so a per-column `REVOKE ... FROM authenticated` would block coaches too.

**How to apply:** Any data that must be coach-only (internal notes, flags, scores you don't want the subject to see) goes in a **separate table with a coach-only RLS policy** — `USING ((auth.jwt() -> 'user_metadata' ->> 'role') = 'coach')`, and **no** athlete policy, so non-coaches get zero rows. Pattern shipped: `coach_member_notes` (S407). Writes go through service-role coach routes (best-effort so a note write never fails the parent action); the coach browser read relies on the coach RLS policy. **Never** rely on "the UI doesn't show it" — RLS is the guarantee. **Verify** by reading the table with the anon key: it must return 0 rows even when a row exists. Security holds regardless of the coach policy being subtly wrong (worst case: coaches don't see it — a visible functional bug, never a leak).
...
**Why:** RM-test lift sessions (Back Squat / Front Squat / Pendlay Testing) silently lost athletes' weights and PRs. TWO independent causes, confirmed with data:
- **Weights** (`wod_section_results.weight_result`) — lift sections stored `scoring_fields.load:false`, and the S338 edit-cleanup in `useWODOperations.ts` nulls `weight_result` when `load` flips true→false on save. So editing/renaming a lift WOD wiped every athlete's weight. (Nulled 07-May.)
- **Lift records** (`lift_records`) — wiped by the **April historical lift-import project (S313–S315)**, NOT by any app action. The S315 log states "DB had been reset since S314 import (only 53 records total)." A bulk reset/re-seed during that messy multi-session import destroyed the live-entered class records, then re-seeded from Chris's historical-PR master JSON, which didn't contain the March testing-day results. Confirmed athletes' bookings stayed `confirmed` (the reset touched only `lift_records`), which is why no booking/no-show/cancel path was involved — Chris was right that he removed nobody.
...
2. **Never run a destructive bulk op (TRUNCATE / reset / unscoped DELETE / restore-overwrite) on a table that holds live user data** (`lift_records`, `wod_section_results`, `benchmark_results`, `bookings`, …). Imports must be **INSERT-only, deduped, and scoped** (see `scripts/import-athlete-lift-records.ts` — it only inserts). If a reset really seems needed: `npm run backup` first, then scope the DELETE to a `source` filter, never the whole table. This is what failed in April.
3. **`scoreCleanup` (booking removal) must never delete `lift_records`** — defensive, not the cause of the April loss but still correct: a removed person is a no-show with no result, so deleting PRs on removal can only ever fire in error. It clears only `wod_section_results`. Bad records → manual delete-lift on the athlete page.
4. **Detection net:** an RM result in `wod_section_results` (weighted, registered athlete) with no matching `lift_records` row = a silent loss. `scripts/check-wsr-liftrecord-parity.ts` (the integrity check) flags it; run it with the periodic orphan checks. April's loss went unnoticed ~2 months because nothing compared the two tables.
5. **After any historical DB restore, re-check two things:** whiteboard rows for athletes who registered *after* the backup (→ leaderboard duplicates, delete the whiteboard row), and scores entered *after* the backup date (→ missing, manual fill). Recovery-script templates: `scripts/sweep-rm-lift-weight-loss.ts`, `restore-*`, `rebuild-*`.
