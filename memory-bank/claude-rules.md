...
`bookings`, `weekly_sessions`, `wods`, `wod_section_results`, `lift_records`, `benchmark_results`, `reactions`, `athlete_achievements`, `programming_plan_items`, **`exercises`** (added S408 — it's a catalogue, not a log, so it grows slowly, but it was already at **716/1000** when checked and nearly every read of it is unfiltered `select('*')`. When it trips, nothing errors: the Movement Library just loses its alphabetical tail and Planner/analysis frequency maps quietly under-count). As of 2026-05-13 the gym has ~2,000 bookings and grows ~330/month, so every one of these tables either has crossed or will cross 1,000 rows within months.
