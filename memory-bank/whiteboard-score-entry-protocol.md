...
6. **Write (INSERT-only, deduped, scoped; dry-run first):**
   - **⚠️ `section_id` MUST be `` `${section.id}-content-0` `` — NOT the bare id (S399).** The app's save route stores it with the `-content-0` suffix ([save/route.ts:215](../app/api/score-entry/save/route.ts#L215)) and the **athlete leaderboard only matches that form**. A bare `section-…` id still shows in the coach modal but is **invisible on the leaderboard** — a silent half-failure. (`publish_sections`, by contrast, stores the *base* id with no suffix.) Don't copy a prior script blind: verify its `section_id` carries the suffix.
