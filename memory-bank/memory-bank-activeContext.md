...
**Version:** 195
**Updated:** 2026-05-05 (Session 335 — booking-window countdown on Book a Class cards + checkpoint vs close checklist split)
...
**Session 335 (2026-05-05 — Opus 4.7) — BOOKING-WINDOW COUNTDOWN ON BOOK-A-CLASS CARDS + CHECKPOINT VS CLOSE CHECKLIST SPLIT:**
- **Book a Class cards now show "Closes in Xd Yh / Xh Ym / Xm" under the capacity row** while still bookable. Gray normally, **amber under 2h**, **red under 30m**. Countdown ticks every 60s (`nowMs` state). Existing "Locked" pill replaces it once the lock instant passes.
- **Per-session-type lock-lead minutes now exposed publicly.** [app/api/booking-rules/public/route.ts](app/api/booking-rules/public/route.ts) extended to also return `auto_lock_lead_minutes` (global) + `session_type_lock_minutes` (array of overrides from `booking_rules_by_session_type`). [app/member/book/page.tsx](app/member/book/page.tsx) consumes both; `lockAtMs = sessionStartInstant - leadMinutes * 60_000`.
- **Latent UTC vs Berlin bug fixed in `effectivelyLocked` check** — was using `new Date(\`${date}T${time}\`)` (runtime-local). Switched to `sessionStartInstant`. See landmine.
- **Workflow: split the session-close checklist into checkpoint + close.** [Chris Notes/AA frequently used files/1-mid-session-checkpoint-checklist.md](Chris%20Notes/AA%20frequently%20used%20files/1-mid-session-checkpoint-checklist.md) NEW (build + backup + commit + push + light memory-bank touch; no Next Session Kickoff rewrite, no project-history file). Old checklist renamed to [2-session-close-checklist.md](Chris%20Notes/AA%20frequently%20used%20files/2-session-close-checklist.md). Cue phrases: "**checkpoint**" → file 1, "**close session**" → file 2.
- **Threshold caveat:** countdown amber/red breakpoints (2h / 30m) are first-pass — same shape as the S334 Statistics dimming thresholds. May need tuning if athletes report the warning fires too early/late.
