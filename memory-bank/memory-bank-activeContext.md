...
**Version:** 198
**Updated:** 2026-05-05 (Session 336 close — exercise display_name "Barbell" prefix strip + 10-card badge actual-bookings rewrite + counter-mismatch ⚠ glyph + unbooked-whiteboard probe)
...
**First action:** Continue manually booking the remaining 8 ten-card athletes from the [scripts/probe-unbooked-whiteboard-athletes.ts](scripts/probe-unbooked-whiteboard-athletes.ts) output (Anton 12, Max 8, Ole 5, Fabian 4, Leopold 3, Adrian 1, Kim 1, Bettina 1 — Rosita's 6 already done). After each athlete, glance at their MemberCard chip — the new ⚠ glyph means the counter doesn't match real bookings (manual override or stale). Open TenCardModal → hit Recalc → Save to clear the warning, OR leave it if the counter is intentionally an override (Rosita's case: pre-app sessions on the same card).
...
**Older sessions (57-331):** See `project-history/` folder.
