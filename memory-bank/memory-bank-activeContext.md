...
**Version:** 200
**Updated:** 2026-05-07 (Session 338 close — leaderboard ignores disabled scoring fields (read+write+toggle-off+cleanup) + cancel-booking finds athlete-self-entered scores via auth user id resolution)
...
**First action:** Verify the AKBS Deadlift leaderboard fix on production. Open the WOD's leaderboard for "WOD Pt.3" (the AKBS section) — Chris's 47/20 entry should now rank above Madeleine's 48/12 (both T2) since the masked Sc1 / Rx scaling values no longer drive the chain. While there, scan a couple of other previously-affected workouts (Back Squat Testing, BFS 5x5, Strict Movements/KBOHC) to confirm the cleanup landed cleanly. If anything looks off, run `npx tsx scripts/cleanup-stale-scoring-fields.ts` (dry run) to see if any stale rows resurfaced.

**Files to open first if continuing code work:**
- [utils/leaderboard-utils.ts](utils/leaderboard-utils.ts) — `maskDisabledFields` is the new centerpiece. Both `bestResultPerUser` and `rankSectionResults` now accept an optional `scoringFields` arg and mask load/scaling slots that the section says are off.
- [hooks/coach/useWODOperations.ts](hooks/coach/useWODOperations.ts) — section-edit save now diffs scoring_fields per kept section; any field that flipped `true → false` triggers a NULL update on existing wod_section_results rows for that section.
- [hooks/coach/useBookingManagement.ts](hooks/coach/useBookingManagement.ts) `handleCancelBooking` — now resolves auth user id via `/api/coach/resolve-auth-user` before the OR cleanup.
- [scripts/cleanup-stale-scoring-fields.ts](scripts/cleanup-stale-scoring-fields.ts) — read-only by default; pass `--apply` to run. Walks every WOD, NULLs columns where `scoring_fields` says the field is off.

**Carry-over status:**
- ✅ S338 leaderboard fix shipped — read-time mask + write-time guard (both save paths) + section-editor toggle-off + 146-row cleanup applied. Ranker now ignores disabled scaling/load slots; new saves can't introduce stale data; coach edits clear existing data.
- ✅ S338 cancel-booking fix shipped — coach Remove Booking now finds athlete-self-entered scores via the new `/api/coach/resolve-auth-user` endpoint. Lift_records cleanup follows automatically via the existing chained delete.
- ⏳ S338 Chris's test Sumo DL row — the stale wod_section_results + lift_record from his earlier test remain. Either delete via Lifts tab manually, or run the booking-cancel flow now that it's fixed.
- ⏳ S336 retroactive booking pass — 6 of 41 missing bookings done (Rosita). 35 remaining across 8 athletes (Anton 12, Max 8, Ole 5, Fabian 4, Leopold 3, Adrian 1, Kim 1, Bettina 1). Manual flow via Session Management modal increments the counter correctly when status lands as `confirmed`.
- ✅ S337 chip + modal polish shipped — chip uses `ten_card_total ?? 10`, mismatch mirrors counter with ⚠ glyph, modal distinguishes full vs near-full, tooltip explains the recorded + manually-added split.
...
- ✅ S334 iOS Safari login fix — confirmed working.
...
**Older sessions (57-333):** See `project-history/` folder.
