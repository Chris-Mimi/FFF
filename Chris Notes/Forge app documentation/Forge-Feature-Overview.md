...
- **Wellpass tab — minimum check-ins enforced automatically** — Wellpass requires a minimum number of weekly check-ins (3 for single-person passes, 6 for spouse-shared passes) to maintain the gym's reimbursement. Coaches drag the weekly Wellpass spreadsheet into the new Wellpass tab and the app imports every week's check-in counts, auto-links household names to registered athletes, and surfaces under-attending households at the top of the list. Any household that fell below threshold last week is automatically capped at **1 booking per household per week** — so a 3-person Wellpass family doesn't get 3× the allowance of a single-member household. The cap applies across all linked members (spouse + kids on the same pass), whoever books first wins. Per-household override mode: `auto` (paying-app subscribers are exempt), `always exempt`, or `always enforce`. Per-member manual block/unblock available on every tracked row — the coach has the final word at any time, regardless of computed status. **Missing athletes auto-block too:** if a tracked athlete stops appearing in the Excel entirely, the import treats it as zero check-ins for that week — they don't slip through by going dark. **Pause tracking when an athlete is injured or away** — one-click Pause/Resume per household with a reason field; paused rows render faded with a "paused" badge, their lifetime Score is hidden, the Excel sync skips them on every import, and the 1-booking-per-week cap is lifted (so an injured athlete doing mobility class isn't penalised, and the rest of the household can still book). Resume restores everything in one click. The name-linker handles German export quirks (last-name-first vs first-name-last) automatically, so spelling variations no longer require manual cleanup. Sort the list by urgency, app-payer status, or alphabetically. The Excel sync rebuilds the whole picture each Sunday — no double bookkeeping, the app becomes the master view over time
