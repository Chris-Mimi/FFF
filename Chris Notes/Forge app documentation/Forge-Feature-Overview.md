...
- **Saved rep-scheme presets for lifts** — in the Configure Sets/Reps panel, save any lift's set-up (a straight 5×5 @ 80%, a 7-set percentage ramp, or a 1/3/5/10RM test) under a name and re-apply it in one click next time you program that lift. Presets are scoped per lift (Bench Press shows only your Bench Press presets) and sync across your devices, so a build-up you dialled in once is always a tap away
- **Keep table layout (monospace)** — paste a formatted table (e.g. a strength scheme with load / reps / rest columns) straight into a section and tick **Keep table layout** — that section then renders in a fixed-width font everywhere it's shown (your gym display screen, the calendar, previews) so the columns stay lined up instead of collapsing. It's off by default and per-section, so nothing else about your existing workouts changes. A live preview under the box shows exactly how it'll look
...
- **Recency dots right in the exercise picker** — when you open the Movement Library to build or edit a workout, every item on all four tabs (Exercises, Lifts, Benchmarks, Forge) carries a small coloured dot showing how long since you last programmed it — green (recent) through grey (months ago) to "never" — with the exact date on hover. It's the same at-a-glance usage colour-coding as the Planner, brought straight into the create/edit modal, so you can spot a fresh, stale, or never-used movement without leaving the workout you're writing
- **Movement Info bar — coaching notes and demo videos where you're actually writing** — as you type a workout, Forge scans your section text, recognises the movements in it, and collects them into a single bar at the top of the create/edit modal. Click a movement's **name** to read the library's full written description — the setup, the cues, the scaling levels — without leaving the workout or opening the Library. Click the **play icon** to watch the demo video, exactly as before, in one click. It means movement mechanics live once, in the library, instead of being copied into every workout that uses them — leaving the session's Intent/Stimulus and Notes fields free for what's actually specific to *this* session
...
- **Mute an athlete selection** — built up a group of athletes but want to browse everything for a moment without losing the list? Hit **mute** (next to "clear") and the selection stays put but stops filtering; **unmute** brings it straight back. The muted state is remembered too, so a carefully-built group survives a quick detour
...
- **Park inactive members** — a dedicated **Parked** tab lets you tuck away members who rarely attend (e.g. an out-of-town friend who drops in once a year) so they stop cluttering the Active, At-Risk, Subscriptions, and 10-Card lists and their count badges. One click **Park** from the Active tab, one click **Restart** to bring them back exactly as they were. Parking only hides — their booking and login access is untouched, so a parked member can still drop in any time without you un-parking them first. **Add a reason** — parking (and blocking) prompts for an optional short note on *why*, shown on the Parked/Blocked row so you remember the context months later; you can also add or edit the note inline on any existing row. These notes are strictly coach-only — stored so no athlete can ever read their own
- **"A parked member is back" alert** — because a parked member keeps full booking access, the moment one books a class again you get a notification ("Parked athlete is back — Restart them?") so you can welcome them and Restart them into the active lists, instead of only noticing weeks later
...
- **Private events — plan special sessions off the radar** — a one-click **Private** toggle in the Session Management modal for anything that isn't a normal class: a summer sports festival, a school demo, a one-off workshop, a team-building event. A private session is hidden from athletes (never shows in the booking app, never bookable) **and** — unlike a plain Hide — its exercises are pulled out of all your planning data: they won't appear in the Workouts movement search, the Movements filter, the Planner, the Movement Tracking grid, the exercise toolkit, or any frequency counts. This keeps a special event from skewing "what has the gym been programming" analytics when none of your athletes actually did it. The event still lives on your coach calendar (marked with a "Private" badge) and stays findable — searching its **name** brings it up, and a dedicated **"Private events"** filter under the search box lists every private session at once so you never have to remember dates. Private stays private through publishing and edits — one tap is all it takes
- **Undo an accidental cancel — Restore Session** — cancelling a session removes it and cancels every booking, which used to be a one-way door. Now the Session Management modal shows a green **Restore Session** button on any cancelled session: one click reopens it and brings back exactly the bookings the cancel removed, each to its original state (confirmed / waitlisted). Athletes who had cancelled *themselves* before the mistake stay cancelled — restore only undoes what the session-cancel did, so nobody gets un-cancelled by accident
...
- **Exercise video links** — attach demonstration and form videos (YouTube or direct) to exercises in the library. Linked videos are automatically surfaced in the workout builder's Movement Info bar whenever those exercises appear in a workout
...
- **Lock morning sessions the evening before** — a dedicated rule for early classes: any session starting before a cutoff time (e.g. before noon) automatically stops taking bookings at a fixed time the previous evening (e.g. 20:00), *regardless of session type*. This solves the common case where morning and evening classes share a type (e.g. "WOD") — the per-type lead-time lock can't isolate mornings, but this can. One toggle plus two time pickers on the Booking Rules tab; evening sessions are untouched. Enforced server-side (booking + cancel) and shown to athletes on the booking page, exactly like the lead-time lock
- **Lock badge on the calendar** — coach calendar cards show a 🔒 Locked badge on any upcoming session that's closed for booking, whether you locked it by hand or a rule auto-locked it. The tooltip says which. No more wondering whether a session is still open — it's visible at a glance without opening the session
...
- **Lapsed warnings stay until you act** — a member who's lapsed and hasn't renewed stays on the Subscriptions Due banner indefinitely, so nobody quietly falls off your radar. A row only leaves when you **Renew** them or explicitly **dismiss** it — it never disappears on its own after a set number of days
...
- **Friendly grace period for cash athletes** — a cash-paying athlete who forgets to pay isn't cut off the instant their month runs out. They get a **7-day grace window** of continued full access, and a gentle in-app reminder banner — *"Your membership is due, please pay to keep your access — X days left"* — appears across the athlete app and the booking page, starting 2 days before their renewal date and through the whole grace week. The banner is dismissable (per day) and only shows for cash members: athletes on auto-renewing card subscriptions never see it, since their payment is automatic. After the grace week passes, access lapses as before
